  ...
  private func loadPeopleFromPlist() {
      ...
      peopleObjects = NSArray(contentsOfFile: peoplePath) as? [[String : String]]  // chaining of optional bindings.
    {
      for object in peopleObjects {  // loop through the data from the plist (an array of dictionaries)
        if let firstName = object["FirstName"], lastName = object["LastName"] {  // more comma-separated optional binding
            let person = Person(first: firstName, last: lastName)  // create an instance of Person
...
/*  Changed to use new, more readable, chained optinal construct below.
